...
def main():
    ...
    with st.sidebar:
        ...
        # File Operations for traditional images
        ...
        with st.expander("📂 DICOM File Upload", expanded=True):
            dicom_file = st.file_uploader("Upload single DICOM image", type=["dcm"])

        # 3D Volume & MPR Upload
        with st.expander("📑 3D Volume & MPR", expanded=True):
            volume_files = st.file_uploader("Upload multiple DICOM files for 3D volume", accept_multiple_files=True, type=["dcm"])
